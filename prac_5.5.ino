...
void setup() {
  ...
  while (test == 1){
    ...
    for(int i = 0; i < whilecount + 1; i++){ //trying to print out the whole array
      Serial.println(athletelist[i]);
      Serial.println(athletetimes[i]); 
    }
    int highest;
    for(int i = 1; i < whilecount; ++i){ //trying to find the largest number in the array
      if (athletelist[i] > athletelist[i+1] ){
        highest = athletelist[i];
      }
    }
